"""Reviewed template: test whether 100 epochs closes the Alpha5 w8 gap on ECG200."""
...
SEEDS=[42,43,44,45,46]; EPOCHS=100; LR=1e-3; K=[1,2]
...
candidate=summary["alpha5_w8"]
gate_margin=10
confirmed=candidate["aggregate_qant_correct"] >= control["aggregate_qant_correct"]-gate_margin
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"AI-Researcher-w8-100epoch-confirmation","experiment_id":"PNN-v1-W8-100Epoch-Confirmation","timestamp_utc":datetime.now(timezone.utc).isoformat(),"backend":"qant-cpu/software-simulation","dataset":{"name":"ECG200","train_shape":list(Xtr.shape),"test_shape":list(Xte.shape)},"configuration":{"seeds":SEEDS,"epochs":EPOCHS,"learning_rate":LR,"batch_size":32,"frequencies":K,"candidates":CANDIDATES,"preregistered_gate":"alpha5_w8.aggregate_qant_correct >= alpha5_w16_control.aggregate_qant_correct - 10"},"rows":rows,"summary":summary,"success_criteria_met":confirmed,"decision":"w8_boundary_confirmed" if confirmed else "w8_boundary_not_confirmed","notes":"Reviewed 100-epoch confirmation template. Same fresh preregistered seeds for w8 and w16 control. Software/simulation backend only."}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result_w8_100epoch_confirmation.json").write_text(json.dumps(payload,indent=2)+chr(10))

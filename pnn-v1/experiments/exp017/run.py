"""PNN-v1 Exp017: redundant Q.ANT readout robustness test on TwoLeadECG."""
...
class PNN(nn.Module):
 def __init__(self,nheads):
  super().__init__(); self.blocks=nn.ModuleList([FourierBlock(16,16) for _ in range(5)]); self.heads=nn.ModuleList([FourierBlock(80,2) for _ in range(nheads)])
...
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"PNN-v1-Proposal017","experiment_id":"PNN-v1-Exp017","timestamp_utc":datetime.now(timezone.utc).isoformat(),"backend":"qant-cpu/software-simulation","dataset":{"name":"TwoLeadECG","train_shape":list(Xtr.shape),"test_shape":list(Xte.shape)},"configuration":{"seeds":SEEDS,"epochs":EPOCHS,"learning_rate":LR,"batch_size":32,"frequencies":K,"candidates":CANDIDATES,"selection":"none; preregistered redundant-readout intervention"},"rows":rows,"summary":summary,"success_criteria_met":bool(success),"intervention_decision":"supported" if success else "not_supported","notes":"Shared architecture rule frozen. Redundant candidate averages three independently parameterized Q.ANT-compatible heads in reference and Q.ANT inference. Software/simulation backend only.","environment":{"python":platform.python_version(),"torch":torch.__version__,"numpy":np.__version__,"aeon":aeon.__version__}}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result017.json").write_text(json.dumps(payload,indent=2)+chr(10))

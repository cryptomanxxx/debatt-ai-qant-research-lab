"""Reviewed template: confirm Alpha5 w8 compression boundary on ECG200."""
...
SEEDS=[121,131,141,151,161]; EPOCHS=50; LR=1e-3; K=[1,2]
...
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"AI-Researcher-w8-confirmation","experiment_id":"PNN-v1-W8-Confirmation","timestamp_utc":datetime.now(timezone.utc).isoformat(),"backend":"qant-cpu/software-simulation","dataset":{"name":"ECG200","train_shape":list(Xtr.shape),"test_shape":list(Xte.shape)},"configuration":{"seeds":SEEDS,"epochs":EPOCHS,"learning_rate":LR,"batch_size":32,"frequencies":K,"candidates":CANDIDATES,"preregistered_gate":"alpha5_w8.aggregate_qant_correct >= alpha5_w16_control.aggregate_qant_correct - 10"},"rows":rows,"summary":summary,"success_criteria_met":confirmed,"decision":"w8_boundary_confirmed" if confirmed else "w8_boundary_not_confirmed","notes":"Reviewed confirmation template. Same fresh preregistered seeds for w8 and w16 control. Software/simulation backend only."}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result_w8_confirmation.json").write_text(json.dumps(payload,indent=2)+chr(10))

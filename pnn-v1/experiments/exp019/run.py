"""PNN-v1 Exp019: SonyAIBORobotSurface1 validation of redundant-readout numerical robustness."""
...
CANDIDATES={"control":1,"redundant3":3}
...
class PNN(nn.Module):
 def __init__(self,nheads):
  super().__init__(); self.blocks=nn.ModuleList([FourierBlock(16,16) for _ in range(4)]); self.heads=nn.ModuleList([FourierBlock(64,2) for _ in range(nheads)])
...
Xtr,ytr0=load_classification("SonyAIBORobotSurface1",split="TRAIN"); Xte,yte0=load_classification("SonyAIBORobotSurface1",split="TEST")
Xtr0,Xte0=as2d(Xtr),as2d(Xte)
Xtr,Xte=Xtr0[:,3:67],Xte0[:,3:67]
...
control=summary["control"]; red=summary["redundant3"]
success=red["mean_absolute_logit_error"]<control["mean_absolute_logit_error"] and red["mean_qant_accuracy"]>=control["mean_qant_accuracy"]-0.01
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"PNN-v1-Proposal019","experiment_id":"PNN-v1-Exp019","timestamp_utc":datetime.now(timezone.utc).isoformat(),"backend":"qant-cpu/software-simulation","dataset":{"name":"SonyAIBORobotSurface1","train_shape":list(Xtr.shape),"test_shape":list(Xte.shape)},"configuration":{"seeds":SEEDS,"epochs":EPOCHS,"learning_rate":LR,"batch_size":32,"frequencies":K,"candidates":CANDIDATES,"selection":"none; preregistered SonyAIBO redundant-readout numerical-robustness validation"},"rows":rows,"summary":summary,"success_criteria_met":bool(success),"intervention_decision":"supported" if success else "not_supported","notes":"Frozen Exp013 SonyAIBORobotSurface1 adaptation; only readout redundancy differs. Primary criterion is lower logit MAE with Q.ANT accuracy within 1 pp of control. Software/simulation backend only.","environment":{"python":platform.python_version(),"torch":torch.__version__,"numpy":np.__version__,"aeon":aeon.__version__}}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result019.json").write_text(json.dumps(payload,indent=2)+chr(10))

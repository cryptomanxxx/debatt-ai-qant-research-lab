"""PNN-v1 Exp011: full-TRAIN confirmation of w16 against active Alpha3."""
...
control=summary["alpha3_w8"]; compact=summary["w16_candidate"]; diff=compact["total_qant_correct_count"]-control["total_qant_correct_count"]
success=diff>=10 and compact["std_qant_accuracy"]<=control["std_qant_accuracy"]+0.01
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"PNN-v1-Proposal011","experiment_id":"PNN-v1-Exp011","timestamp_utc":datetime.now(timezone.utc).isoformat(),"backend":"qant-cpu/software-simulation","dataset":{"name":"ECG200","train_shape":list(Xtr.shape),"test_shape":list(Xte.shape),"split":"canonical TRAIN/TEST"},"configuration":{"seeds":SEEDS,"epochs":EPOCHS,"learning_rate":LR,"batch_size":32,"preprocessing":"raw archive values","candidates":CANDIDATES,"frequencies":[1,2],"selection":"none; frozen full-TRAIN confirmation"},"results":rows,"summary":summary,"correct_count_difference_w16_minus_alpha3":int(diff),"success_criteria_met":bool(success),"promotion_decision":"promote_alpha4_w16" if success else "retain_alpha3","environment":{"python":platform.python_version(),"torch":torch.__version__,"numpy":np.__version__,"aeon":aeon.__version__},"notes":"Full-TRAIN w16 confirmation against active Alpha3. TEST does not tune or select models. Software/simulation backend only."}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result011.json").write_text(json.dumps(payload,indent=2)+"\n"); print(json.dumps({"summary":summary,"difference":diff,"success":success,"promotion":payload["promotion_decision"]},indent=2))

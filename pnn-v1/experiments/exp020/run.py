"""PNN-v1 Exp020: Alpha5 model-revision promotion test on ECG200."""
...
CANDIDATES={"alpha4":1,"alpha5_candidate":3}
...
control=summary["alpha4"]; red=summary["alpha5_candidate"]
control_correct=int(round(sum(r["qant_accuracy"]*len(yte) for r in rows if r["candidate"]=="alpha4")))
red_correct=int(round(sum(r["qant_accuracy"]*len(yte) for r in rows if r["candidate"]=="alpha5_candidate")))
summary["alpha4"]["aggregate_qant_correct"]=control_correct
summary["alpha5_candidate"]["aggregate_qant_correct"]=red_correct
success=red["mean_absolute_logit_error"]<control["mean_absolute_logit_error"] and red_correct>=control_correct-10 and red["parameter_count"]<=10000
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"PNN-v1-Proposal020","experiment_id":"PNN-v1-Exp020","timestamp_utc":datetime.now(timezone.utc).isoformat(),"backend":"qant-cpu/software-simulation","dataset":{"name":"ECG200","train_shape":list(Xtr.shape),"test_shape":list(Xte.shape)},"configuration":{"seeds":SEEDS,"epochs":EPOCHS,"learning_rate":LR,"batch_size":32,"frequencies":K,"candidates":CANDIDATES,"selection":"preregistered Alpha4 versus Alpha5 promotion gate"},"rows":rows,"summary":summary,"success_criteria_met":bool(success),"promotion_decision":"promote_alpha5" if success else "retain_alpha4","notes":"Alpha5 candidate changes only Alpha4 final readout redundancy. Promotion requires lower logit MAE, no more than 10 aggregate Q.ANT correct predictions lost out of 1000, and <=10000 parameters. Software/simulation backend only.","environment":{"python":platform.python_version(),"torch":torch.__version__,"numpy":np.__version__,"aeon":aeon.__version__}}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result020.json").write_text(json.dumps(payload,indent=2)+chr(10))
print(json.dumps({"summary":summary,"success":success,"decision":payload["promotion_decision"]},indent=2))

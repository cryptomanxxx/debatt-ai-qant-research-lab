"""PNN-v1 Exp003: preregistered residual-head architecture test."""
...
class PNN(nn.Module):
 def __init__(self,residual=False):
  ...
  self.blocks=nn.ModuleList([FourierBlock(8,4) for _ in range(12)]); self.head=FourierBlock(48,2)
  self.skip=nn.Linear(48,2,bias=False) if residual else None
 def features(self,x): return torch.cat([b(x[:,i*8:(i+1)*8]) for i,b in enumerate(self.blocks)],1)
 def forward(self,x):
  h=self.features(x); return self.head(h)+(self.skip(h) if self.skip is not None else 0)
 def qant(self,x):
  a=x.detach().numpy(); h=np.concatenate([b.qant(a[:,i*8:(i+1)*8]) for i,b in enumerate(self.blocks)],1)
  out=self.head.qant(h)
  if self.skip is not None: out=out+h@self.skip.weight.detach().numpy().T
  return out

def nparams(m): return sum(p.numel() for p in m.parameters())
def train(seed,residual):
 torch.manual_seed(seed); np.random.seed(seed); m=PNN(residual)
...
candidates={"alpha1_control":False,"alpha2_residual_head":True}; validation=[]; trained={}
for seed in SEEDS:
 for cid,residual in candidates.items():
  m=train(seed,residual); trained[(seed,cid)]=m
...
selected="alpha2_residual_head" if success else "alpha1_control"
...
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"PNN-v1-Proposal003","experiment_id":"PNN-v1-Exp003",
 ...
 "candidates":["alpha1_control","alpha2_residual_head"],"test_selection_boundary":"candidate frozen from validation before TEST evaluation"},
 ...
 "notes":"Architecture-level residual-head test. Canonical TEST is confirmatory only and is evaluated after validation-based selection. Software/simulation backend only."}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result003.json").write_text(json.dumps(payload,indent=2)+"\n")

"""PNN-v1 Exp007: integer-count confirmation of frozen Alpha2 candidate."""
...
rows=[]
for seed in SEEDS:
 for cid,ctor in {"alpha1_w4":lambda:PNN(4),"alpha2_w8":lambda:PNN(8)}.items():
  ...
  row.update({"qant_accuracy":float((qp==yte).mean()),"qant_correct_count":int((qp==yte).sum()),"prediction_disagreements":int((rp!=qp).sum()),"mean_absolute_logit_error":float(err.mean())})
  rows.append(row)

summary={}
for cid in ("alpha1_w4","alpha2_w8"):
 ...
 s.update({"mean_qant_accuracy":float(np.mean([r["qant_accuracy"] for r in rs])),"std_qant_accuracy":float(np.std([r["qant_accuracy"] for r in rs])),"total_qant_correct_count":int(sum(r["qant_correct_count"] for r in rs)),"mean_prediction_disagreements":float(np.mean([r["prediction_disagreements"] for r in rs])),"mean_absolute_logit_error":float(np.mean([r["mean_absolute_logit_error"] for r in rs]))})
 summary[cid]=s
a=summary["alpha1_w4"]; b=summary["alpha2_w8"]
correct_count_difference=b["total_qant_correct_count"]-a["total_qant_correct_count"]
success=correct_count_difference>=10 and b["std_qant_accuracy"]<=a["std_qant_accuracy"]+0.01
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"PNN-v1-Proposal007","experiment_id":"PNN-v1-Exp007",
 ...
 "models":["alpha1_w4","alpha2_w8"],"selection":"none; frozen integer-count confirmation"},
 "results":rows,"summary":summary,"correct_count_difference_alpha2_minus_alpha1":int(correct_count_difference),"success_criteria_met":bool(success),"promotion_decision":"promote_alpha2" if success else "retain_alpha1",
 "environment":{"python":platform.python_version(),"torch":torch.__version__,"numpy":np.__version__,"aeon":aeon.__version__},
 "notes":"Integer-count tie-breaker confirmation using frozen Exp006 protocol. All models trained on the same 100 canonical TRAIN cases; TEST does not tune or select any model. Software/simulation backend only."}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result007.json").write_text(json.dumps(payload,indent=2)+"\n")
print(json.dumps({"summary":summary,"correct_count_difference":correct_count_difference,"success_criteria_met":success,"promotion_decision":payload["promotion_decision"]},indent=2))

"""PNN-v1 Exp004: preregistered hierarchical cross-window mixer test."""
...
class PNN(nn.Module):
 def __init__(self,mixer=False):
  super().__init__(); self.mixer_enabled=mixer
  self.blocks=nn.ModuleList([FourierBlock(8,4) for _ in range(12)])
  self.mixers=nn.ModuleList([FourierBlock(8,4) for _ in range(6)]) if mixer else None
  self.head=FourierBlock(24 if mixer else 48,2)
 def features(self,x):
  h=[b(x[:,i*8:(i+1)*8]) for i,b in enumerate(self.blocks)]
  if self.mixers is not None: h=[self.mixers[i](torch.cat([h[2*i],h[2*i+1]],1)) for i in range(6)]
  return torch.cat(h,1)
 def forward(self,x): return self.head(self.features(x))
 def qant(self,x):
  a=x.detach().numpy(); h=[b.qant(a[:,i*8:(i+1)*8]) for i,b in enumerate(self.blocks)]
  if self.mixers is not None: h=[self.mixers[i].qant(np.concatenate([h[2*i],h[2*i+1]],1)) for i in range(6)]
  return self.head.qant(np.concatenate(h,1))

def nparams(m): return sum(p.numel() for p in m.parameters())
def train(seed,mixer):
 torch.manual_seed(seed); np.random.seed(seed); m=PNN(mixer)
...
candidates={"alpha1_control":False,"alpha2_hierarchical_mixer":True}; validation=[]; trained={}
for seed in SEEDS:
 for cid,mixer in candidates.items():
  m=train(seed,mixer); trained[(seed,cid)]=m
...
success=b["mean_validation_qant_accuracy"]>a["mean_validation_qant_accuracy"] and b["std_validation_qant_accuracy"]<=a["std_validation_qant_accuracy"]+0.01
...
payload={"schema_version":1,"project":"Debatt-AI Photonic Neural Network v1","proposal_id":"PNN-v1-Proposal004","experiment_id":"PNN-v1-Exp004",
 ...
 "candidates":["alpha1_control","alpha2_hierarchical_mixer"],"test_selection_boundary":"candidate frozen from validation before TEST evaluation"},
 ...
 "notes":"Architecture-level hierarchical cross-window mixer test. Canonical TEST is confirmatory only and is evaluated after validation-based selection. Software/simulation backend only."}
Path("pnn-v1/results").mkdir(parents=True,exist_ok=True); Path("pnn-v1/results/result004.json").write_text(json.dumps(payload,indent=2)+"\n")

"""Experiment 008: full-MNIST validation of selected expansion architectures."""
...
SEEDS=[11,22,33]
TOPOLOGIES={"h32_512":[32,512],"h64_512":[64,512],"h128_512":[128,512],
            "h128":[128],"h256":[256],"h512":[512]}
...
for cid,w in TOPOLOGIES.items():
 if params(w)>MAX_PARAMS: raise SystemExit(f"Budget violation: {cid} has {params(w)} parameters > {MAX_PARAMS}")
...
payload={"schema_version":1,"experiment_id":"exp008_full_mnist_validation",
 ...
 "notes":"Full-MNIST validation. Q.ANT CPU backend validates execution/correctness only; no photonic performance claim."}
Path("local_results").mkdir(exist_ok=True)
Path("local_results/exp008_full_mnist_validation.json").write_text(json.dumps(payload,indent=2)+"\n")

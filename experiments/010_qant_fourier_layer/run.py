"""Experiment 010: Q.ANT-native Fourier/KAN-style layer vs ReLU controls on Fashion-MNIST."""
...
SEEDS=[11,22,33]
CANDIDATES={
 "qfourier_g1":{"kind":"fourier","grid_size":1},
 "qfourier_g2":{"kind":"fourier","grid_size":2},
 "h64_512":{"kind":"relu","widths":[64,512]},
 "h128_512":{"kind":"relu","widths":[128,512]},
 "h256":{"kind":"relu","widths":[256]},
}
...
for seed in SEEDS:
 for cid,cfg in CANDIDATES.items():
  ...
  rows.append({"seed":seed,"candidate_id":cid,"kind":cfg["kind"],"parameter_count":nparams(cid),
...
for cid in CANDIDATES:
 ...
 summary[cid]={"mean_qant_accuracy":float(np.mean(vals)),"std_qant_accuracy":float(np.std(vals)),
  "min_qant_accuracy":float(np.min(vals)),"max_qant_accuracy":float(np.max(vals)),"parameter_count":nparams(cid),
...
payload={"schema_version":1,"experiment_id":"exp010_qant_fourier_layer",
 ...
 "notes":"QFourier formulation follows Q.ANT's official example: train with torch.cos, evaluate with calc_kan_layer_fprop. Q.ANT CPU backend only; no photonic performance claim."}
Path("local_results").mkdir(exist_ok=True)
Path("local_results/exp010_qant_fourier_layer.json").write_text(json.dumps(payload,indent=2)+"\n")

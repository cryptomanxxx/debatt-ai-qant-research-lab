"""Experiment 006: multi-seed replication of selected Exp005 topology pattern.

Runtime configuration is supplied by the guarded job through QANT_JOB_CONFIG.
"""
...
TOPOLOGIES={"h32_256":[32,256],"h64_64_256":[64,64,256],"h128_256":[128,256],
...
for cid,widths in TOPOLOGIES.items():
 if params(widths)>MAX_PARAMS: raise SystemExit(f"Budget violation: {cid} exceeds max_parameters")
...
for seed in SEEDS:
 for cid,widths in TOPOLOGIES.items():
  ...
  row={"seed":seed,"candidate_id":cid,"widths":widths,"parameter_count":params(widths),
       "reference_accuracy":rc/n,"qant_accuracy":qc/n,"prediction_disagreements":dis}
  rows.append(row); print(row)

summary={}
for cid in TOPOLOGIES:
 ...
 summary[cid]={"mean_qant_accuracy":float(np.mean(vals)),"std_qant_accuracy":float(np.std(vals)),
               ...
               "parameter_count":params(TOPOLOGIES[cid])}

payload={"schema_version":1,"experiment_id":"exp006_multiseed_replication",
 ...
 "notes":"Multi-seed replication. Q.ANT CPU backend validates execution/correctness only; no photonic performance claim."}
Path("local_results").mkdir(exist_ok=True)
Path("local_results/exp006_multiseed_replication.json").write_text(json.dumps(payload,indent=2)+"\n")
print("\nSUMMARY"); print(json.dumps(summary,indent=2))
print("Saved local_results/exp006_multiseed_replication.json")
